...
const G_LIGHT = {
  ...
  sep: "rgba(0,0,0,0.1)", accent: "#1c1c1c", ok: "#188038",
          ...
          <button key={app.id} onClick={() => onOpen(app.id)} style={{ background: "none", border: "none", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 8, padding: 0 }}>
            <div style={{ width: 56, height: 56, borderRadius: 28, background: th.surface2, border: `1px solid ${th.sep}`, display: "flex", alignItems: "center", justifyContent: "center", position: "relative", color: th.text }}>
              ...
              {app.id === "messages" && unread > 0 && (
                <div style={{ position: "absolute", top: -3, right: -3, minWidth: 17, height: 17, borderRadius: 9, background: th.accent, color: "#111111", fontSize: 10.5, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 4px" }}>{unread}</div>
              )}
            </div>
          ...
          <React.Fragment>
              ...
              {active.msgs.map((m, i) => {
                ...
                return (
                  <div key={m.id} style={{ display: "flex", justifyContent: out ? "flex-end" : "flex-start", animation: i === active.msgs.length - 1 ? "nbBubbleIn 0.3s " + SPRING : "none" }}>
                    <div style={{ maxWidth: "76%", padding: "7px 11px", borderRadius: 8, background: out ? th.accent : th.surface2, border: out ? "none" : `1px solid ${th.sep}`, color: out ? "#111111" : th.text, fontSize: 13.5, lineHeight: 1.4, fontWeight: out ? 500 : 400 }}>{m.t}</div>
                  </div>
                );
              })}
              {active.typing && (
            ...
            <div style={{ padding: "8px 10px 34px", display: "flex", gap: 7, borderTop: `1px solid ${th.sep}` }}>
              ...
              <button onClick={() => { if (draft.trim()) { send(active.id, draft.trim()); setDraft(""); } }} style={{ width: 36, height: 36, borderRadius: 8, border: "none", cursor: "pointer", background: draft.trim() ? th.accent : th.surface2, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <Icon d={P.send} size={16} color={draft.trim() ? "#111111" : th.text3} sw={2.4} />
              </button>
            </div>
          </React.Fragment>
        )}
...
function GToggle({ on, onChange, th }) {
  return (
    <button onClick={onChange} style={{ width: 40, height: 22, borderRadius: 4, border: `1px solid ${on ? th.accent : th.sep}`, cursor: "pointer", background: on ? th.accent : "transparent", position: "relative", transition: "all 0.2s", flexShrink: 0 }}>
      <div style={{ position: "absolute", top: 2, left: on ? 19 : 2, width: 16, height: 16, borderRadius: 3, background: on ? "#111111" : th.text3, transition: `left 0.2s ${SPRING}` }} />
    </button>
  );
}
...
function Mark1Demo() {
  ...
  return (
    <div style={{
      ...
      boxShadow: "inset 0 1px 1px rgba(255,255,255,0.16), 0 26px 70px rgba(0,0,0,0.55), 0 0 60px rgba(255,255,255,0.06)",
    }}>
      {/* antenna bands */}
      ...
      <div style={{ position: "absolute", right: -2.5, top: "35%", width: 3, height: 34, borderRadius: 2, background: "#4a4a4a" }} />
...
function INullberryApp({ th, nodes, feed }) {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, background: th.name === "dark" ? "radial-gradient(110% 60% at 80% 0%, rgba(255,255,255,0.09), transparent 60%), #05080b" : "radial-gradient(110% 60% at 80% 0%, rgba(255,255,255,0.16), transparent 60%), #eef4f6", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "12px 14px 46px" }}>
        ...
        <Card>
          ...
          <div style={{ fontFamily: MONO, fontSize: 10.5, color: th.text, wordBreak: "break-all", background: th.name === "dark" ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.07)", border: `1px solid ${th.name === "dark" ? "rgba(255,255,255,0.18)" : "rgba(0,0,0,0.2)"}`, borderRadius: 8, padding: "8px 10px", animation: "nbFadeIn 0.4s ease" }} key={myAddr}>
            {myAddr}
          </div>
